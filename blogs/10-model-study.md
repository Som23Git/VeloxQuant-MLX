...
$$c_i = \frac{\int_{b_{i-1}}^{b_i} x \cdot f(x)\, dx}{\int_{b_{i-1}}^{b_i} f(x)\, dx}$$
...
$$\tilde{k} = D \cdot \text{diag}(r) \cdot \text{WHT}(k)$$
...
$$\langle q, k \rangle \approx \langle q, \hat{k}_{\text{MSE}} \rangle + \|r\| \cdot \frac{\pi}{2m} \cdot \sum_{j=1}^{m} z_j \cdot \text{sign}(\langle S_j, q \rangle)$$
