"""Offline-synthetic benchmark for KVzip-adapted reconstruction-reliance retention.

The paper's contribution (arXiv:2505.23416, NeurIPS 2025 Oral) is a *retention
rule*: keep a constant-size cache by ranking stored tokens according to how much
the model relies on them to *reconstruct its own context* — query-agnostic — and
evict the least-relied-upon. So the honest thing to measure is NOT "KVzip beats
everything" but "does reconstruction-reliance retention keep the region the model
relies on to reconstruct its context, where cumulative (H2O-style) scoring keeps
stale early heavy hitters — and stay neutral when there is no such shift?"

Two geometries:

  1. **reconstruction_shift** — an early block of heavy traffic on axis A, then a
     cluster of reconstruction-critical tokens whose axis-B signal is WEAK and
     per-token NOISY but that mutually reinforce (each attends to the others under
     the reconstruction probe). Cumulative (H2O) scoring is dominated by the early
     axis-A mass and retains stale tokens (~0 axis-B retention); a single latest
     token (probe="latest" == TOVA-adapted) is a noisy proxy and only partly
     re-targets; the full-context reconstruction probe aggregates the mutual
     reinforcement and reliably surfaces axis B — so it retains the axis-B region
     at a materially higher rate. Primary observable: the **reconstruction-
     critical (axis-B) retention rate**.
  2. **flat** — all traffic on one axis from token 0. Here there is nothing to
     re-target and KVzip should be roughly neutral vs cumulative. Reporting this
     control is the point: the rule is not a free win.

Arms at matched budget: KVzip probe∈{latest, context} (probe="latest" ==
latest-token / TOVA-adapted reference), an H2O-adapted cumulative cross-check,
and seeded random eviction. Metrics: reconstruction-critical retention rate
(primary) and probe-query output perturbation (secondary, reported as-is) — the
same perturbation metric family as the MorphKV/Keyformer/Q-Filters benchmarks.

KVzip is deterministic (no RNG), so arms are not seed-averaged over noise; each
geometry is evaluated on several data seeds and the mean reported.
...
LLaMA3.1/Qwen2.5/Gemma3) are the paper's, on trained models — not reproduced here.

Usage
-----
    python benchmark_scripts/benchmark_kvzip.py
...
def _synthetic(S: int, geometry: str, seed: int):
    ...
    axis_a carries the early heavy traffic; axis_b carries the reconstruction-
    critical cluster that the reconstruction probe (and the probe queries) rely
    on. Keeping axis-B tokens is what minimizes probe perturbation under a
    ...
    if geometry == "reconstruction_shift":
        # Early 70%: heavy traffic on axis A (cumulative heavy hitters).
        early_end = int(S * 0.70)
        k[:early_end, 0] = 3.0
        # Late 30%: a reconstruction-critical cluster on axis B. Each token's
        # axis-B component is WEAK relative to per-token noise, so a single latest
        # token (probe="latest") is an unreliable proxy. But the cluster mutually
        # reinforces: under the full-context reconstruction probe, axis-B tokens
        # attend to one another, so aggregating over the probe surfaces axis B —
        # this is where the context probe earns its keep over the latest-token
        # (TOVA-adapted) reference.
...
def _relevant_retention(kept_k: mx.array, axis: np.ndarray) -> float:
    """Fraction of kept rows strongly aligned with the reconstruction-critical axis."""
    ...
    # component, so a kept token counts as "reconstruction-critical" if it carries
    # a real axis-B signal rather than just noise.
...
def _run_once(S: int, budget: int, geometry: str, seed: int) -> dict:
    ...
    # KVzip probe arms — deterministic; averaged over data seeds.
    for probe in PROBES:
        perts, rets, mss = [], [], []
        for ds in DATA_SEEDS:
            ...
            mss.append(ms)
        row[f"pert_{probe}"] = round(float(np.mean(perts)), 5)
        row[f"retain_{probe}"] = round(float(np.mean(rets)), 3)
        row[f"ms_{probe}"] = round(float(np.mean(mss)), 2)
...
def main() -> None:
    ...
    for geom in GEOMETRIES:
        ...
        ctx_ret = np.mean([r["retain_context"] for r in rows])
    ...
    print("   early heavy hitters, while KVzip's reconstruction-reliance ranking re-targets")
    print("   the cache toward the region the model relies on to reconstruct its context.")
    print("   The downstream probe PERTURBATION is a noisier, regime-dependent secondary")
    print("   effect, reported as-is rather than cherry-picked. Under FLAT geometry there is")
    print("   nothing to re-target and the rule is roughly neutral — it is a retention")
    print("   policy, not a free win. probe=latest is bit-for-bit the latest-token")
