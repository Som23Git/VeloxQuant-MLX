...
def build_vlm_caches(model, bits: int, use_rvq: bool = False, seed: int = 42):
    ...
    for i, layer in enumerate(layers):
        ...
        if n_kv is None or hd is None:
            if args is not None:
                n_kv = n_kv or getattr(args, "num_key_value_heads",
                                        getattr(args, "num_attention_heads", 1))
                hd = hd or getattr(args, "head_dim", None) or (
...
def _read_model_cfg(model):
    ...
    layers = getattr(model, "layers", None) or getattr(
        ...
        getattr(getattr(model, "language_model", None), "model", None), "layers", None
    ) or []
    if n_layers is None:
        n_layers = len(layers)
    for layer in layers:
...
def _generate_figures_v3(
    ...
    head_dim, n_kv_heads, n_layers,
    token_counts, fp16_full,
    sm_fp16, sm_2b, sm_3b, sm_4b, sm_rvq2, sm_rvq1,
    cos_rvq2, snr_rvq2,
    cos_rvq1, snr_rvq1,
    responses,
):
    ...
    fig2.suptitle(f"Quality vs Bits — {model_label} (head_dim={head_dim})",
                  fontsize=13, fontweight="bold")
    ax_s.plot(br, snrs, color=PALETTE_V3["4bit"], marker="s", lw=2.2, ms=7,
              label="single-pass TurboQuant")
    ax_c.plot(br, coss, color=PALETTE_V3["4bit"], marker="s", lw=2.2, ms=7,
              label="single-pass TurboQuant")
    ax_s.scatter([2], [snr_rvq2], color=PALETTE_V3["rvq2"], marker="*", s=280,
                 zorder=5, label="RVQ 2-bit ★")
    ax_c.scatter([2], [cos_rvq2], color=PALETTE_V3["rvq2"], marker="*", s=280,
                 zorder=5, label="RVQ 2-bit ★")
    ax_s.scatter([1], [snr_rvq1], color=PALETTE_V3["rvq1"], marker="*", s=280,
                 zorder=5, label="RVQ 1-bit ★")
    ax_c.scatter([1], [cos_rvq1], color=PALETTE_V3["rvq1"], marker="*", s=280,
                 zorder=5, label="RVQ 1-bit ★")
    ax_s.axhline(10, color="green",  ls="--", lw=1.5, alpha=0.7, label="10 dB (near-lossless)")
    ax_c.axhline(0.90, color="green",  ls="--", lw=1.5, alpha=0.7, label="0.90 near-lossless")
    ax_c.axhline(0.80, color="orange", ls="--", lw=1.5, alpha=0.7, label="0.80 degraded")
    for ax, lbl in [(ax_s, "SNR (dB)"), (ax_c, "Cosine Similarity")]:
        ax.set_xlabel("Bit-width"); ax.set_ylabel(lbl)
        ax.set_xticks(br); ax.legend(fontsize=8); sns.despine(ax=ax)
    ax_s.set_title("Signal-to-Noise Ratio",     fontsize=12, fontweight="bold")
    ax_c.set_title("Cosine Similarity",         fontsize=12, fontweight="bold")
    ax_c.set_ylim(0.4, 1.05)
    fig2.tight_layout()
    ...
    ax_a.plot(token_counts, to_mb(fp16_full),   color=PALETTE_V3["fp16"],  lw=2.5, marker="o",  ms=5, label="fp16 K+V")
    ax_a.plot(token_counts, to_mb(tq2_  + val), color=PALETTE_V3["2bit"],  lw=2.0, marker="D",  ms=4, label="TQ 2-bit")
    ax_a.plot(token_counts, to_mb(tq3_  + val), color=PALETTE_V3["3bit"],  lw=2.0, marker="s",  ms=4, label="TQ 3-bit")
    ax_a.plot(token_counts, to_mb(tq4_  + val), color=PALETTE_V3["4bit"],  lw=2.0, marker="^",  ms=4, label="TQ 4-bit")
    ax_a.plot(token_counts, to_mb(rvq2_ + val), color=PALETTE_V3["rvq2"],  lw=2.5, marker="*",  ms=8, label="RVQ 2-bit ★")
    ax_a.plot(token_counts, to_mb(rvq1_ + val), color=PALETTE_V3["rvq1"],  lw=2.5, marker="P",  ms=7, label="RVQ 1-bit ★")
    ...
    ax_r.plot(token_counts, r2, color=PALETTE_V3["2bit"],  lw=2.0, marker="D",  ms=4, label="TQ 2-bit")
    ax_r.plot(token_counts, r3, color=PALETTE_V3["3bit"],  lw=2.0, marker="s",  ms=4, label="TQ 3-bit")
    ax_r.plot(token_counts, r4, color=PALETTE_V3["4bit"],  lw=2.0, marker="^",  ms=4, label="TQ 4-bit")
    ax_r.plot(token_counts, rr, color=PALETTE_V3["rvq2"],  lw=2.5, marker="*",  ms=8, label="RVQ 2-bit ★")
    ax_r.plot(token_counts, r1, color=PALETTE_V3["rvq1"],  lw=2.5, marker="P",  ms=7, label="RVQ 1-bit ★")
    ax_r.axhline(1.0, color="grey", ls="--", lw=1, alpha=0.6)
    ...
    for ax, (sm, label, col) in zip(axes4, panels):
        ...
        ax.plot(np.arange(N_k), sm_fp16, color=PALETTE_V3["fp16"],
    ...
    ax_C = fig6.add_subplot(gs[1, 0])
    ax_C.plot(br, coss, color=PALETTE_V3["4bit"], marker="s", lw=2.2, ms=7,
              label="single-pass TQ")
    ax_C.scatter([2], [cos_rvq2], color=PALETTE_V3["rvq2"], marker="*",
                 s=280, zorder=5, label="RVQ 2-bit ★")
    ax_C.scatter([1], [cos_rvq1], color=PALETTE_V3["rvq1"], marker="*",
                 s=280, zorder=5, label="RVQ 1-bit ★")
    ...
    ax_D = fig6.add_subplot(gs[1, 1])
    ax_D.plot(token_counts, to_mb(fp16_full),   color=PALETTE_V3["fp16"],  lw=2.5, marker="o",  ms=5, label="fp16")
    ax_D.plot(token_counts, to_mb(tq2_  + val), color=PALETTE_V3["2bit"],  lw=2.0, marker="D",  ms=4, label="TQ 2-bit")
    ax_D.plot(token_counts, to_mb(tq4_  + val), color=PALETTE_V3["4bit"],  lw=2.0, marker="^",  ms=4, label="TQ 4-bit")
    ax_D.plot(token_counts, to_mb(rvq2_ + val), color=PALETTE_V3["rvq2"],  lw=2.5, marker="*",  ms=8, label="RVQ 2-bit ★")
    ax_D.plot(token_counts, to_mb(rvq1_ + val), color=PALETTE_V3["rvq1"],  lw=2.5, marker="P",  ms=7, label="RVQ 1-bit ★")
    ax_D.set_xscale("log", base=2); ax_D.set_xticks(token_counts)
    ax_D.set_xticklabels([f"{t//1024}K" if t >= 1024 else str(t) for t in token_counts], fontsize=7)
    ax_D.set_xlabel("Context length"); ax_D.set_ylabel("Memory (MB)")
    ...
    ax_E.legend(fontsize=8, ncol=3); sns.despine(ax=ax_E)
...
def run_benchmark_v3_from_results(results_by_config, out_dir, model_label,
                                   head_dim, n_kv_heads, n_layers):
    ...
    configs = ["fp16\nbaseline", "TQ\n2-bit", "TQ\n3-bit",
               "TQ\n4-bit", "RVQ\n2-bit ★", "RVQ\n1-bit ★"]
    colors  = [PALETTE_V3["fp16"], PALETTE_V3["2bit"], PALETTE_V3["3bit"],
               PALETTE_V3["4bit"], PALETTE_V3["rvq2"], PALETTE_V3["rvq1"]]
    ...
    def _attn_tq(bits):
        ...
        ev   = qt.encode(k_mx); k_hat = qt.decode(ev)
        sc   = np.array(k_hat @ q_mx.T).flatten()
        return np.exp(sc) / np.exp(sc).sum()
    ...
    _generate_figures_v3(
        ...
        token_counts=token_counts, fp16_full=fp16_full,
        sm_fp16=sm_fp16, sm_2b=sm_2b, sm_3b=sm_3b, sm_4b=sm_4b,
        sm_rvq2=sm_rvq2, sm_rvq1=sm_rvq1,
        cos_rvq2=cos_rvq2, snr_rvq2=snr_rvq2,
