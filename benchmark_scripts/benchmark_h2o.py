"""Offline-synthetic benchmark for H2O-adapted KV cache.

Sweeps (seq_len, h2o_budget, h2o_n_sink) on synthetic fp16 K/V data,
measuring update latency and compression ratio. No model required.

Usage
-----
    python benchmark_scripts/benchmark_h2o.py
...
def main() -> None:
    print("H2O-adapted KV Cache — offline synthetic benchmark")
    ...
    for seq_len, budget, n_sink in product(SEQ_LENS, BUDGETS, N_SINKS):
        ...
        results.append(row)
        print(
            f"{row['seq_len']:>8}  {row['h2o_budget']:>8}  {row['h2o_n_sink']:>7}  "
            f"{row['kept_tokens']:>6}  {row['compression_ratio']:>7.2f}x  "
            f"{row['latency_ms_mean']:>9.3f}  {row['latency_ms_min']:>7.3f}"
        )

    out_path = Path(__file__).parent / "h2o_benchmark_results.json"
